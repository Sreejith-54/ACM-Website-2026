...
const StudentCard = (props) => {
  const { src, name, position, instagram, linkedin, github } = props;
  const [hover, setHover] = useState(false);

  return (
    <div
      className={`relative h-[310px] w-full max-w-[260px] overflow-hidden rounded-lg border border-white/10 bg-white/[0.06] p-3 shadow-xl shadow-black/30 transition-all duration-300 ${hover ? 'scale-[1.03] border-accent/60 bg-accent/10' : 'scale-100'}`}
    ...
    >
      ...
      <AnimatePresence>
        {hover && (
          ...
          >
            {instagram && (
              <a className="rounded-md bg-white/10 p-2 transition hover:bg-white/20" href={instagram} target="_blank" rel="noreferrer">
                <Image src="/instagram.png" width={28} height={28} alt="Instagram" />
              </a>
            )}
            {linkedin && (
